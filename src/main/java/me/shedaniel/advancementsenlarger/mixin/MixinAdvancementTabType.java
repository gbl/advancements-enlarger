...
public abstract class MixinAdvancementTabType implements AdvancementTabTypeHooks {
    ...
    @Override
    public void ae_drawBackground(MatrixStack stack, DrawableHelper drawable, int x, int y, boolean selected, int index) {
        int i = this.u;
        if (index > 0) {
            i += this.width;
        }
        ...
        drawable.drawTexture(stack, x + (this.width + 2) * index, y + -this.height + 4, i, j, this.width, this.height);
    }
    
    @Override
    public void ae_drawIcon(MatrixStack stack, int x, int y, int index, ItemRenderer itemRenderer, ItemStack icon) {
        int i = x + (this.width + 2) * index + 6;
        int j = y + -this.height + 4 + 9;
        itemRenderer.renderInGuiWithOverrides((LivingEntity)null, icon, i, j);
    }
